...
import lazismuh from "../assets/image/lazismuh.jpg"
import {
  MapContainer,
  Marker,
  Popup,
...
const Home = () => {
  ...
  const maps = useMemo(() => {
    console.log(search)
    return `https://maps.google.com/maps?q=${search}&t=&z=15&ie=UTF8&iwloc=&output=embed`
  }, [search])
  ...
  const fetchKelurahan = async () => {
    try {
      const { data: response } = await axios.get('/api/kelurahan')
  ...
  useEffect(() => {
    axios.get(`/api/${tipe}/${search}`).then((res) => {
  ...
  return (
    <div>
      ...
      >
        <img src={lazismuh} style={{ width: '100px', marginRight: '20px' }} />
        <div>
      ...
      <div className="box">
        <div className="content" style={{ marginRight: '30px' }}>
          ...
          <div style={{ marginTop: '20px' }}>
            <iframe src={maps} width="100%" height="450" allowFullScreen frameborder="0"></iframe>
          </div>
        </div>
      ...
      {lura.length > 0 ? (
        <>
          ...
          >
            ...
            <ul>
              <li>
                Perbandingan amil dan muzakki : 1 amil dapat menerima dana dari{' '}
                {Math.floor(count.muzakki / count.amil)} muzakki
              </li>
              <li>
                Perbandingan amil dan mustahik : 1 amil dapat memberikan dana untuk{' '}
                {Math.floor(count.mustahik / count.amil)} mustahik
              </li>
            </ul>
            <br />
            ...
            <ul>
              ...
              <li>
                1 amil memerlukan waktu {waktuRata * Math.floor(count.mustahik / count.amil)} menit
                atau{' '}
                {Math.round(((waktuRata * Math.floor(count.mustahik / count.amil)) / 60) * 100) /
                  100}{' '}
                jam untuk melayani {Math.floor(count.mustahik / count.amil)} mustahik
              </li>
              <li>
                {count.amil} amil memerlukan waktu{' '}
                {waktuRata * Math.floor(count.mustahik / count.amil) * count.amil} menit atau{' '}
                {Math.round(
                  ((waktuRata * Math.floor(count.mustahik / count.amil) * count.amil) / 60) * 100,
                ) / 100}{' '}
                jam atau{' '}
                {Math.round(
                  (waktuRata * Math.floor(count.mustahik / count.amil) * count.amil) / 60 / 24,
                )}{' '}
                hari untuk melayani {count.mustahik} mustahik
              </li>
            </ul>
          </div>
